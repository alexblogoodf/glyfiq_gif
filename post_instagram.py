...
POSTED_FILE = "output/instagram_posted_history.json"
PENDING_FILE = "output/instagram_pending.json"

# 5 вариантов: коннектор + полный текст (ссылка текстом без https + 5 хештегов)
TEMPLATES = [
    ("and",
...
def load_posted():
    if os.path.exists(POSTED_FILE):
        try:
            ...
            print(f"⚠️ Не удалось прочитать историю Instagram: {e}")
...
def cmd_prepare():
    if os.path.exists(PENDING_FILE):
        print("ℹ️ Уже есть отложенный пост для Instagram. Подготовка пропущена.")
        return

    print("🎬 Подготовка поста для Instagram...")
    ...
    if candidate is None:
        print("😴 Все GIF уже запощены в Instagram. Завершаемся.")
        return

    # MP4 должен быть уже готов скриптом для Threads — сами НЕ конвертируем
    gif_path = candidate.get("gif_path", "")
    mp4_path = gif_path.rsplit(".", 1)[0] + ".mp4" if gif_path else ""

    if not mp4_path or not os.path.exists(mp4_path):
        print(f"⏳ MP4 для GIF № {candidate.get('gif_number')} ещё не готов (ждём скрипт Threads).")
...
def cmd_post():
    if not os.path.exists(PENDING_FILE):
        print("😴 Нет отложенного поста для Instagram.")
        return
    ...
    print(f"🚀 Публикуем в Instagram: {video_url}")
    ...
    if ok or already_posted:
        ...
        print("💾 История Instagram обновлена.")
    else:
